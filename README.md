...
word [here: a placeholder] may be followed by an integer lex_id from 1 to 15 . The lex_id is used to distinguish different senses of the same word within a lexicographer file. The lexicographer assigns lex_id values, usually in ascending order, although there is no requirement that the numbers be consecutive. The default is 0 , and does not have to be specified. 
#Source: https://wordnet.princeton.edu/documentation/wninput5wn
...
# Der Verwis auf das Lexicographer-File müsste der Verweis auf die Sense-Gruppe sein
